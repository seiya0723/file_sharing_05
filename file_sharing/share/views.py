        ...
        if form.is_valid():
            ...
            if mime in ALLOWED_MIME:
                ...
                if "application/pdf" == document.mime:
                    ...
                    print(str(settings.MEDIA_ROOT) + "/" + str(document.content))
                    print(full_path)

                    image   = PSDImage.open(settings.MEDIA_ROOT + "/" + str(document.content))
                    image.composite().save(full_path)

                    #画像を保存する。
                    """
                    image   = PSDImage.open(document.content.url)
